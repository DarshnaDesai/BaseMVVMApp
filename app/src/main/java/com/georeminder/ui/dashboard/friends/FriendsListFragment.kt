 ...
 */
class FriendsListFragment : BaseFragment<FriendsViewModel>(), BaseBindingListAdapter.ItemClickListener<FriendRequest> {
    ...
    override fun onAttach(context: Context?) {
        ...
        val networkLocalComponent = DaggerNetworkLocalComponent
                ...
                .build()
        networkLocalComponent.injectFriendsListFragment(this)
    }

    private fun init() {
        setAdapter()
        viewModel.callFriendListApi("1")
        setObservables()
    }
    ...
    private fun setAdapter() {
        ...
        val adapter = FriendRequestsAdapter(context!!, false)
    ...
    private fun updateReminderData(it: RequestStateChange) {
        ...
        if (it.status != RequestStatus.PENDING && it.status != RequestStatus.CANCELLED) {
...
//            adapter.notifyItemRemoved()
    ...
    override fun onItemClick(view: View, data: FriendRequest, position: Int) {
        when (view.id) {
            ...
            else -> startActivityForResult(FriendDetailActivity.newIntent(context!!, data, false,
                    position), AppConstants.EXTRA_REQUEST_CODE)
        }
    }
